  ...
  <div class="mt-28 lg:flex lg:items-center lg:gap-6">
    <div class="border border-cyan-300 px-3 py-5 rounded-lg shadow-lg lg:w-1/2 h-72">
      ...
      <div class="mt-3">
        ...
        <p class="text-zinc-700 mt-3">Số điện thoại: <span class="text-orange-700 font-bold">{{ props.phoneNumber }}</span></p>
        <p class="text-zinc-700 mt-3">Địa chỉ: <span class="text-orange-700 font-bold">{{ props.streetNumber }}, {{ props.street }}, {{ props.ward }}, {{ props.district }}, {{ props.province }}
        </span></p>
      </div>
    </div>
    <div class="border border-cyan-300 px-3 py-5 rounded-lg shadow-lg lg:w-1/2 h-72 mt-4 lg:mt-0">
      ...
      <div class="mt-3">
        <div v-if="props.nameDeliveryStaff" class="flex flex-col items-center justify-center px-10 py-2">
          <img :src="props.urlDeliveryStaff" class="w-32 h-32 object-cover rounded-full text-center"/>
        ...
        <div v-else class="flex flex-col items-center justify-between px-10 py-10">
          <p class="text-zinc-600">Chưa cập nhật nhân viên vận chuyển. Xin quý khách vui lòng đợi cho đến khi nhân viên vận chuyển nhận đơn</p>
        </div>
      </div>
    </div>
  </div>
...
const props = defineProps({
  ...
  district: String,
  province: String,
  nameDeliveryStaff: String,
  phoneNumberDeliveryStaff: String,
  urlDeliveryStaff: String,
})

console.log(props.urlDeliveryStaffL)
