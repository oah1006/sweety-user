  ...
  <div class="fixed top-0 left-0 w-full flex h-screen justify-center items-center transition duration-700 ease-in-out">
    ...
    <div class="w-1/3 mx-auto rounded-lg shadow-md z-50">
      ...
      <div class="bg-orange-100 w-full h-full flex justify-end items-center gap-8 px-8 py-4">
        ...
        <button @click="deleteDeliveryAddress(props.itemId)" class="bg-orange-400 text-white px-6 py-2 rounded-md">Đồng ý</button>
      </div>
    </div>
  </div>
</template>
...
const emits = defineEmits(['close', 'delete-delivery-address'])
...
function deleteDeliveryAddress(hasIdDeliveryAddressDelete) {
  emits('delete-delivery-address', hasIdDeliveryAddressDelete)
  closeModal()
}
