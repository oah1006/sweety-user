<template>
  ...
  <LayoutDetailProduct v-if="!isLoadingPage" class="mt-14">
    <template #image>
      <div>
        <img :src="url" class="w-full h-[550px] object-cover rounded-lg" />
      </div>
    </template>
    <template #information>
      <div>
        <p class="text-4xl font-bold text-zinc-600">{{ product.name }}</p>
      ...
      <div class="mt-6">
        ...
        <div class="flex items-center gap-2 mt-2">
          <div v-for="size in product.product_variants" :key="size">
            <a @click="addVariantObject(size.id, size.unit_price)" :class="[size.id == cart.product_variant_id ? 'bg-orange-500' : '']"  class="px-8 py-2 border border-zinc-300 rounded-lg flex items-center gap-2 text-zinc-600">
    ...
    <template #description>
      <div class="border-y border-zinc-300 py-6">
        <div>
    ...
    <template #related-product>
      <div class="py-6">
        ...
        <div class="grid grid-cols-4 mt-4 justify-center">
          <div @click="useClickRedirectDetail(product.id)" v-for="product in relatedProduct" :key="product.id" class="cursor-pointer">
            <img :src="product.attachment[0]?.url" class="w-64 h-64 object-cover rounded-lg" />
            <p class="font-medium text-2xl mt-3 font-bold">{{ product.name }}</p>
