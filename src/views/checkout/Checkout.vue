    ...
    <LayoutCheckout v-if="!isLoadingPage">
      <template #address-customer>
        <div>
          <div class="flex items-center gap-8 border border-zinc-200 rounded-lg px-4 py-2">
            ...
            <div class="flex flex-col">
              ...
              <p v-else>Hãy chọn địa chỉ giao hàng dến vị trí của bạn</p>
              <a @click="showDeliveryAddressModal" class="text-cyan-500 underline cursor-pointer ml-auto">Chọn địa chỉ</a>
            </div>
          </div>
          <div v-if="isDeliveryAddressModal" class="mt-1 border border-zinc-300 rounded-lg">
            <div v-if="delivery_addresses" v-for="delivery_address in delivery_addresses" :key="delivery_address.id">
            ...
            <div v-else class="px-3 py-4 flex flex-col items-center gap-2">
              <p class="text-zinc-600">Bạn chưa tạo bất kỳ địa chỉ nào mới tai hệ thống Sweety</p>
              <router-link class="px-2 py-1 bg-cyan-500 text-white rounded-lg" :to="{ name: 'create-delivery-address' }" >Tạo địa chỉ mới</router-link>
            </div>
          </div>
        ...
        <div class="mt-10 border-t border-sold border-zinc-300">
          ...
          <div class="flex items-center gap-8 border border-zinc-200 rounded-lg px-4">
            <div class="w-1/5">
            ...
            <div v-else>
              <p class="text-sm text-zinc-500">Khi bạn chọn địa chỉ để giao hàng, hệ thống sẽ giúp bạn chọn cửa hàng gần nhất</p>
            </div>
          </div>
...
function showDeliveryAddressModal() {
  isDeliveryAddressModal.value = !isDeliveryAddressModal.value
}
